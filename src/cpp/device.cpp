...
JS_METHOD(getDeviceIDs) {
	...
	Napi::Array deviceArray = JS_ARRAY;
	if (!n) {
		RET_VALUE(deviceArray);
	}

	std::unique_ptr<cl_device_id[]> devices(new cl_device_id[n]);
	CHECK_ERR(clGetDeviceIDs(platform, type, n, devices.get(), nullptr));
...
inline Napi::Value getDeviceInfoString(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(
	    clGetDeviceInfo(device_id, param_name, sizeof(char) * 1024, param_value, &param_value_size_ret)
	);
	RET_STR(param_value);
}
...
inline Napi::Value getDeviceInfoPlatform(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_platform_id), &param_value, nullptr));
...
inline Napi::Value getDeviceInfoType(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_device_type), &param_value, nullptr));
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoMemType(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(
	    clGetDeviceInfo(device_id, param_name, sizeof(cl_device_local_mem_type), &param_value, nullptr)
	);
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoCacheType(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(
	    clGetDeviceInfo(device_id, param_name, sizeof(cl_device_mem_cache_type), &param_value, nullptr)
	);
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoCaps(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(
	    clGetDeviceInfo(device_id, param_name, sizeof(cl_device_exec_capabilities), &param_value, nullptr)
	);
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoQueueProps(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(
	    clGetDeviceInfo(device_id, param_name, sizeof(cl_command_queue_properties), &param_value, nullptr)
	);
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoFpConfig(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_device_fp_config), &param_value, nullptr));
	RET_NUM(param_value);
}
...
inline Napi::Value getDeviceInfoMaxWorkItem(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	...
	CHECK_ERR(clGetDeviceInfo(
	    device_id, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(size_t), &max_work_item_dimensions, nullptr
	));

	// get CL_DEVICE_MAX_WORK_ITEM_SIZES array param
	std::unique_ptr<size_t[]> param_value(new size_t[max_work_item_dimensions]);
	CHECK_ERR(clGetDeviceInfo(
	    device_id, param_name, max_work_item_dimensions * sizeof(size_t), param_value.get(), nullptr
	));
...
inline Napi::Value getDeviceInfoBool(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	cl_bool param_value;
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_bool), &param_value, nullptr));
...
inline Napi::Value getDeviceInfoUint(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	cl_uint param_value;
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_uint), &param_value, nullptr));
	RET_NUM(param_value);
}

inline Napi::Value getDeviceInfoUlong(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	cl_ulong param_value;
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(cl_ulong), &param_value, nullptr));
	RET_X64(param_value);
}

inline Napi::Value getDeviceInfoSize(Napi::Env env, cl_device_id device_id, uint32_t param_name) {
	size_t param_value;
	CHECK_ERR(clGetDeviceInfo(device_id, param_name, sizeof(size_t), &param_value, nullptr));
	RET_NUM(param_value);
}
...
JS_METHOD(createSubDevices) {
	...
	ret = clCreateSubDevices(deviceId, &cl_properties.front(), capacity, subDevices.get(), nullptr);
	CHECK_ERR(ret);

	Napi::Array subDevicesArray = JS_ARRAY;
...
JS_METHOD(retainDevice) {
	...
	clGetDeviceInfo(deviceId, CL_DEVICE_PARENT_DEVICE, sizeof(cl_device_id), &parentId, nullptr);
