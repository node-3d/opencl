...
JS_METHOD(waitForEvents) {
	...
	CHECK_ERR(clWaitForEvents((cl_uint)cl_events.size(), &cl_events.front()));
...
JS_METHOD(getEventInfo) {
	...
	switch (param_name) {
		case CL_EVENT_COMMAND_QUEUE: {
			...
			CHECK_ERR(clGetEventInfo(ev, param_name, sizeof(cl_command_queue), &val, nullptr));
			CHECK_ERR(clRetainCommandQueue(val));
			RET_WRAPPER(val);
		}
		case CL_EVENT_CONTEXT: {
			cl_context val;
			CHECK_ERR(clGetEventInfo(ev, param_name, sizeof(cl_context), &val, nullptr));
			CHECK_ERR(clRetainContext(val));
			RET_WRAPPER(val);
		}
		case CL_EVENT_COMMAND_TYPE: {
			cl_command_type val;
			CHECK_ERR(clGetEventInfo(ev, param_name, sizeof(cl_command_type), &val, nullptr));
			RET_NUM(val);
		}
...
JS_METHOD(getEventProfilingInfo) {
	...
	switch (param_name) {
		...
		case CL_PROFILING_COMMAND_END: {
			cl_ulong val;
			CHECK_ERR(clGetEventProfilingInfo(ev, param_name, sizeof(cl_ulong), &val, nullptr));
