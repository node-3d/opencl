...
JS_METHOD(createSubBuffer) {
	...
	cl_mem mem = clCreateSubBuffer(buffer, flags, CL_BUFFER_CREATE_TYPE_REGION, &buffer_create_info, &ret);
	CHECK_ERR(ret);
...
JS_METHOD(createImage) {
	...
	image_format.image_channel_order =
	    formatObj.Has("channel_order") ? formatObj.Get("channel_order").ToNumber().Uint32Value() : 0;
	image_format.image_channel_data_type =
	    formatObj.Has("channel_data_type") ? formatObj.Get("channel_data_type").ToNumber().Uint32Value() : 0;
	...
	desc.image_type =
	    descObj.Has("type") ? descObj.Get("type").ToNumber().Uint32Value() : CL_MEM_OBJECT_IMAGE1D;
	desc.image_width = descObj.Has("width") ? descObj.Get("width").ToNumber().Int64Value() : 1;
	desc.image_height = descObj.Has("height") ? descObj.Get("height").ToNumber().Int64Value() : 1;
	desc.image_depth = descObj.Has("depth") ? descObj.Get("depth").ToNumber().Int64Value() : 1;
	desc.image_array_size = descObj.Has("array_size") ? descObj.Get("array_size").ToNumber().Int64Value() : 0;
	desc.image_row_pitch = descObj.Has("row_pitch") ? descObj.Get("row_pitch").ToNumber().Int64Value() : 0;
	desc.image_slice_pitch =
	    descObj.Has("slice_pitch") ? descObj.Get("slice_pitch").ToNumber().Int64Value() : 0;
...
JS_METHOD(getSupportedImageFormats) {
	...
	CHECK_ERR(clGetSupportedImageFormats(context, flags, image_type, 0, nullptr, &numEntries));

	Napi::Array imageFormats = JS_ARRAY;
	if (!numEntries) {
	...
	CHECK_ERR(
	    clGetSupportedImageFormats(context, flags, image_type, numEntries, image_formats.get(), nullptr)
	);

	for (size_t i = 0; i < numEntries; i++) {
		Napi::Object format = JS_OBJECT;
...
JS_METHOD(getMemObjectInfo) {
	...
	switch (param_name) {
		case CL_MEM_TYPE: {
			...
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(cl_mem_object_type), &val, nullptr));
			RET_NUM(val);
		}
		case CL_MEM_FLAGS: {
			cl_mem_flags val;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(cl_mem_flags), &val, nullptr));
			RET_NUM(val);
		}
		case CL_MEM_SIZE:
		case CL_MEM_OFFSET: {
			size_t val;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(size_t), &val, nullptr));
			RET_NUM(val);
		}
		case CL_MEM_MAP_COUNT:
		case CL_MEM_REFERENCE_COUNT: {
			cl_uint val;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(cl_uint), &val, nullptr));
			RET_NUM(val);
		}
		case CL_MEM_HOST_PTR: {
			size_t memSize;
			CHECK_ERR(clGetMemObjectInfo(mem, CL_MEM_SIZE, sizeof(size_t), &memSize, nullptr));
			if (!memSize) {
				RET_NULL;
			}
			void *memPtr;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(void *), &memPtr, nullptr));
			if (!memPtr) {
				RET_NULL;
			}
		...
		case CL_MEM_CONTEXT: {
			cl_context val;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(cl_context), &val, nullptr));
			CHECK_ERR(clRetainContext(val))
			RET_WRAPPER(val);
		}
		case CL_MEM_ASSOCIATED_MEMOBJECT: {
			cl_mem val;
			CHECK_ERR(clGetMemObjectInfo(mem, param_name, sizeof(cl_mem), &val, nullptr));
...
JS_METHOD(getImageInfo) {
	...
	switch (param_name) {
		case CL_IMAGE_FORMAT: {
			cl_image_format val;
			CHECK_ERR(clGetImageInfo(mem, param_name, sizeof(cl_image_format), &val, nullptr));
		...
		case CL_IMAGE_ARRAY_SIZE: {
			size_t val;
			CHECK_ERR(clGetImageInfo(mem, param_name, sizeof(size_t), &val, nullptr));
			RET_NUM(val);
		}
		...
		case CL_IMAGE_NUM_SAMPLES: {
			cl_uint val;
			CHECK_ERR(clGetImageInfo(mem, param_name, sizeof(cl_uint), &val, nullptr));
			RET_NUM(val);
		}
	}
