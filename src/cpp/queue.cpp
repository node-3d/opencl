...
#define GET_EVENT_FLAG(n)                                                                                    \
	...
	cl_event *eventPtr = (!IS_ARG_EMPTY(n) && info[n].ToBoolean().Value()) ? &event : nullptr;

#define GET_BLOCK_FLAG(n)                                                                                    \
	cl_event event = nullptr;                                                                                \
	cl_event *eventPtr = (!IS_ARG_EMPTY(n) && info[n].ToBoolean().Value()) ? nullptr : &event;
...
JS_METHOD(getCommandQueueInfo) {
	...
	switch (param_name) {
		case CL_QUEUE_CONTEXT: {
			cl_context val;
			CHECK_ERR(clGetCommandQueueInfo(clQueue, param_name, sizeof(cl_context), &val, nullptr));
			CHECK_ERR(clRetainContext(val))
			RET_WRAPPER(val);
		}
		case CL_QUEUE_DEVICE: {
			cl_device_id val;
			CHECK_ERR(clGetCommandQueueInfo(clQueue, param_name, sizeof(cl_device_id), &val, nullptr));
			CHECK_ERR(clRetainDevice(val))
			RET_WRAPPER(val);
		}
		case CL_QUEUE_REFERENCE_COUNT: {
			cl_uint val;
			CHECK_ERR(clGetCommandQueueInfo(clQueue, param_name, sizeof(cl_uint), &val, nullptr));
			RET_NUM(val);
		}
		case CL_QUEUE_PROPERTIES: {
			cl_command_queue_properties val;
			CHECK_ERR(
			    clGetCommandQueueInfo(clQueue, param_name, sizeof(cl_command_queue_properties), &val, nullptr)
			);
			RET_NUM(val);
		}
	}
...
JS_METHOD(enqueueFillBuffer) {
	...
	CHECK_ERR(clEnqueueFillBuffer(
	    clQueue, clMem, pattern, len, offset, size, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr
	));
...
JS_METHOD(enqueueFillImage) {
	...
	CHECK_ERR(clEnqueueFillImage(
	    clQueue, image, ptr, origin, region, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr
	));
...
JS_METHOD(enqueueUnmapMemObject) {
	...
	cl_int err;
	err = clEnqueueUnmapMemObject(clQueue, mem, ptr, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr);
	CHECK_ERR(err)
...
JS_METHOD(enqueueTask) {
	...
	CHECK_ERR(clEnqueueTask(clQueue, k, cl_events.size(), &cl_events.front(), eventPtr));
...
JS_METHOD(enqueueMarkerWithWaitList) {
	...
	CHECK_ERR(clEnqueueMarkerWithWaitList(clQueue, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr));
...
JS_METHOD(enqueueBarrierWithWaitList) {
	...
	CHECK_ERR(clEnqueueBarrierWithWaitList(clQueue, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr));
...
JS_METHOD(enqueueAcquireGLObjects) {
	...
	CHECK_ERR(
	    clEnqueueAcquireGLObjects(queue, 1, &mem, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr)
	);
...
JS_METHOD(enqueueReleaseGLObjects) {
	...
	CHECK_ERR(
	    clEnqueueReleaseGLObjects(queue, 1, &mem, (cl_uint)cl_events.size(), &cl_events.front(), eventPtr)
	);
