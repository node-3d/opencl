...
JS_METHOD(createProgramWithBuiltInKernels) {
	...
	cl_program prg = clCreateProgramWithBuiltInKernels(
	    context, (cl_uint)cl_devices.size(), &cl_devices.front(), names.front(), &err
	);
...
JS_METHOD(getProgramInfo) {
	...
	switch (param_name) {
		...
		case CL_PROGRAM_NUM_DEVICES: {
			cl_uint val;
			CHECK_ERR(clGetProgramInfo(prog, param_name, sizeof(cl_uint), &val, nullptr));
			RET_NUM(val);
		}
		case CL_PROGRAM_CONTEXT: {
			cl_context val;
			CHECK_ERR(clGetProgramInfo(prog, param_name, sizeof(cl_context), &val, nullptr));
		...
		case CL_PROGRAM_DEVICES: {
			...
			CHECK_ERR(clGetProgramInfo(prog, param_name, sizeof(cl_device_id) * n, devices.get(), nullptr));
		...
		case CL_PROGRAM_BINARY_SIZES: {
			cl_uint nsizes;
			CHECK_ERR(clGetProgramInfo(prog, CL_PROGRAM_NUM_DEVICES, sizeof(cl_uint), &nsizes, nullptr));

			std::unique_ptr<size_t[]> sizes(new size_t[nsizes]);
			CHECK_ERR(clGetProgramInfo(prog, param_name, nsizes * sizeof(size_t), sizes.get(), nullptr));
		...
		case CL_PROGRAM_SOURCE: {
			...
			CHECK_ERR(clGetProgramInfo(prog, param_name, nchars * sizeof(char), names.get(), nullptr));
			RET_STR(names.get());
		}
	}
...
JS_METHOD(getProgramBuildInfo) {
	...
	switch (param_name) {
		case CL_PROGRAM_BUILD_STATUS: {
			cl_build_status val;
			CHECK_ERR(
			    clGetProgramBuildInfo(prog, device, param_name, sizeof(cl_build_status), &val, nullptr)
			);
			RET_NUM(val);
		}
		...
		case CL_PROGRAM_BUILD_LOG: {
			...
			CHECK_ERR(clGetProgramBuildInfo(prog, device, param_name, 0, nullptr, &param_value_size_ret));
			std::unique_ptr<char[]> param_value(new char[param_value_size_ret]);
			CHECK_ERR(clGetProgramBuildInfo(
			    prog, device, param_name, param_value_size_ret, param_value.get(), nullptr
			));
		...
		case CL_PROGRAM_BINARY_TYPE: {
			...
			CHECK_ERR(
			    clGetProgramBuildInfo(prog, device, param_name, sizeof(cl_program_binary_type), &val, nullptr)
			);
			RET_NUM(val);
		}
	}
