...
JS_METHOD(setKernelArg) {
	...
	if (IS_ARG_EMPTY(2)) {
		...
		size_t nchars = 0;
		CHECK_ERR(clGetKernelArgInfo(kernel, arg_idx, CL_KERNEL_ARG_TYPE_NAME, 0, nullptr, &nchars));
		char *tname = new char[nchars];
		CHECK_ERR(clGetKernelArgInfo(kernel, arg_idx, CL_KERNEL_ARG_TYPE_NAME, nchars, tname, nullptr));
...
JS_METHOD(getKernelInfo) {
	...
	switch (param_name) {
		...
		case CL_KERNEL_FUNCTION_NAME: {
			size_t nchars = 0;
			CHECK_ERR(clGetKernelInfo(kernel, param_name, 0, nullptr, &nchars));
		...
		case CL_KERNEL_REFERENCE_COUNT: {
			cl_uint num = 0;
			CHECK_ERR(clGetKernelInfo(kernel, param_name, sizeof(cl_uint), &num, nullptr));
			RET_NUM(num);
		}
		case CL_KERNEL_CONTEXT: {
			cl_context ctx = 0;
			CHECK_ERR(clGetKernelInfo(kernel, param_name, sizeof(cl_context), &ctx, nullptr));
			CHECK_ERR(clRetainContext(ctx))
			RET_WRAPPER(ctx);
		}
		case CL_KERNEL_PROGRAM: {
			cl_program p = 0;
			CHECK_ERR(clGetKernelInfo(kernel, param_name, sizeof(cl_program), &p, nullptr));
...
JS_METHOD(getKernelArgInfo) {
	...
	switch (param_name) {
		case CL_KERNEL_ARG_ADDRESS_QUALIFIER: {
			...
			CHECK_ERR(clGetKernelArgInfo(
			    kernel, arg_idx, param_name, sizeof(cl_kernel_arg_address_qualifier), &num, nullptr
			));
			RET_NUM(num);
		}
		case CL_KERNEL_ARG_ACCESS_QUALIFIER: {
			...
			CHECK_ERR(clGetKernelArgInfo(
			    kernel, arg_idx, param_name, sizeof(cl_kernel_arg_access_qualifier), &num, nullptr
			));
			RET_NUM(num);
		}
		case CL_KERNEL_ARG_TYPE_QUALIFIER: {
			...
			CHECK_ERR(clGetKernelArgInfo(
			    kernel, arg_idx, param_name, sizeof(cl_kernel_arg_type_qualifier), &num, nullptr
			));
			RET_NUM(num);
		}
		...
		case CL_KERNEL_ARG_NAME: {
			size_t nchars = 0;
			CHECK_ERR(clGetKernelArgInfo(kernel, arg_idx, param_name, 0, nullptr, &nchars));
			std::unique_ptr<char[]> name(new char[nchars]);
			CHECK_ERR(clGetKernelArgInfo(kernel, arg_idx, param_name, nchars, name.get(), nullptr));
			RET_STR(name.get());
		}
	}
...
JS_METHOD(getKernelWorkGroupInfo) {
	...
	switch (param_name) {
		...
		case CL_KERNEL_COMPILE_WORK_GROUP_SIZE: {
			size_t sz[3] = { 0, 0, 0 };
			CHECK_ERR(clGetKernelWorkGroupInfo(kernel, device, param_name, 3 * sizeof(size_t), sz, nullptr));
			Napi::Array szarr = JS_ARRAY;
		...
		case CL_KERNEL_WORK_GROUP_SIZE: {
			size_t sz = 0;
			CHECK_ERR(clGetKernelWorkGroupInfo(kernel, device, param_name, sizeof(size_t), &sz, nullptr));
			RET_NUM(sz);
		}
		case CL_KERNEL_LOCAL_MEM_SIZE:
		case CL_KERNEL_PRIVATE_MEM_SIZE: {
			cl_ulong sz = 0;
			CHECK_ERR(clGetKernelWorkGroupInfo(kernel, device, param_name, sizeof(cl_ulong), &sz, nullptr));
			RET_X64(sz);
		}
	}
