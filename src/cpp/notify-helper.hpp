...
template <typename T> class NotifyHelper {
  public:
	NotifyHelper(Napi::Function callback, Napi::Value userData) {
		Napi::Env env = callback.Env();

		_ref.Reset(JS_OBJECT, 1);
  ...
  private:
	...
	void _notify(T resource) {
		...
		napi_status result = _tsfn.NonBlockingCall([that, resource](Napi::Env env, Napi::Function callback) {
			callback.Call(that->_ref.Value(), { Wrapper::from(env, resource), that->_ref.Get("data") });
		});
		if (result != napi_ok) {
	...
	void _notifyStatus(T resource, cl_int status) {
		NotifyHelper<T> *that = this;
		napi_status result =
		    _tsfn.NonBlockingCall([that, resource, status](Napi::Env env, Napi::Function callback) {
			    callback.Call(
			        that->_ref.Value(),
