...
describe('CommandQueue - Image Copy', () => {
	...
	describe('#enqueueCopyImage', () => {
		it('works with cl.MEM_READ_WRITE images', () => {
			const image1 = cl.createImage(context, cl.MEM_READ_WRITE, imageFormat, imageDesc, null);
			const image2 = cl.createImage(context, cl.MEM_READ_WRITE, imageFormat, imageDesc, null);
			// command queue, cl_image_src, cl_image_dst, origin_src, origin_dst, region
			const ret = cl.enqueueCopyImage(cq, image1, image2, zeroArray, zeroArray, validRegion);
	...
	describe('#enqueueCopyImageToBuffer', () => {
		it('works with read-only buffers', () => {
			...
			const ret = cl.enqueueCopyImageToBuffer(cq, image, buffer, zeroArray, [1, 1, 1], 0);
		...
		it('works with different values of source and destination offsets', () => {
			...
			const ret = cl.enqueueCopyImageToBuffer(cq, image, buffer, [1, 1, 0], [1, 1, 1], 2);
		...
		it('throws cl.INVALID_VALUE if origin is invalid', () => {
			...
			assert.throws(
				() => cl.enqueueCopyImageToBuffer(cq, image, buffer, invalidOrigin, [1, 1, 1], 2),
				cl.INVALID_VALUE,
			);
	...
	describe('#enqueueCopyBufferToImage', () => {
		it('works with read-only buffers', () => {
			...
			const ret = cl.enqueueCopyBufferToImage(cq, buffer, image, 0, zeroArray, [1, 1, 1]);
		...
		it('throws cl.INVALID_VALUE if origin is invalid', () => {
			...
			const invalidOrigin = [1, 1, 1];
			assert.throws(
				() => cl.enqueueCopyBufferToImage(cq, buffer, image, 0, invalidOrigin, [1, 1, 1]),
				cl.INVALID_VALUE,
			);
	...
	describe('#enqueueMapImage', () => {
		...
		it('doesnt throw as we are using the pointer from an event', (_t, done) => {
			const image = cl.createImage(context, 0, imageFormat, imageDesc, null);
			const ret = cl.enqueueMapImage(cq, image, false, cl.MAP_WRITE, zeroArray, [2, 2, 1]);
