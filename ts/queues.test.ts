...
const squareKern = fs
	...
	.toString();
const squareOneKern = fs
	.readFileSync(new URL('../examples/assets/kernels/square_one.cl', import.meta.url))
	.toString();
...
describe('CommandQueue - Common', () => {
	...
	describe('#createCommandQueue', () => {
		it('fails given an invalid property', () => {
			assert.throws(() => cl.createCommandQueue(context, device, -1));
		});
	...
	describe('#enqueueUnmapMemObject', () => {
		it('throws as we are unmapping a non mapped memobject', () => {
			...
			assert.throws(() => cl.enqueueUnmapMemObject(cq, buf, Buffer.alloc(5)));
	...
	describe('#enqueueTask', () => {
		it('works with a valid call', () => {
			U.withProgram(context, squareOneKern, (prg) => {
				...
				const outputsMem = cl.createBuffer(context, cl.MEM_WRITE_ONLY, 4, null);
