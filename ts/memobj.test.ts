...
describe('MemObj', () => {
	...
	describe('#createBuffer', () => {
		...
		it('throws cl.INVALID_BUFFER_SIZE if size is 0', () => {
			assert.throws(() => cl.createBuffer(context, 0, 0), cl.INVALID_BUFFER_SIZE);
		});
		...
		it('throws cl.INVALID_MEM_OBJECT when passed an Array', () => {
			assert.throws(
				() =>
					cl.createBuffer(context, cl.MEM_COPY_HOST_PTR, 8, [
						1, 2, 3, 4, 5, 6, 7, 8,
	...
	describe('#createSubBuffer', () => {
		it('throws if buffer is not valid', () => {
			assert.throws(() =>
				cl.createSubBuffer({} as unknown as cl.TClMem, cl.MEM_READ_WRITE, 0, 2),
			);
		});

		it('creates a subBuffer - empty flags', () => {
			const subBuffer = cl.createSubBuffer(buffer, 0, 0, 2);

			cl.releaseMemObject(subBuffer);
		});

		it('creates a subBuffer - cl.MEM_WRITE_ONLY', () => {
			const subBuffer = cl.createSubBuffer(buffer, cl.MEM_WRITE_ONLY, 0, 2);

			cl.releaseMemObject(subBuffer);
		});

		it('creates a subBuffer - cl.MEM_READ_ONLY', () => {
			const subBuffer = cl.createSubBuffer(buffer, cl.MEM_READ_ONLY, 0, 2);
		...
		it('creates a subBuffer - cl.MEM_HOST_WRITE_ONLY', () => {
			const subBuffer = cl.createSubBuffer(buffer, cl.MEM_HOST_WRITE_ONLY, 0, 2);

			cl.releaseMemObject(subBuffer);
		});

		it('creates a subBuffer - cl.MEM_HOST_READ_ONLY', () => {
			const subBuffer = cl.createSubBuffer(buffer, cl.MEM_HOST_READ_ONLY, 0, 2);

			cl.releaseMemObject(subBuffer);
		});

		it('creates a subBuffer - cl.MEM_HOST_NO_ACCESS', () => {
			const subBuffer = cl.createSubBuffer(buffer, cl.MEM_HOST_NO_ACCESS, 0, 2);
	...
	describe('#createImage', () => {
		...
		it('creates an image - cl.MEM_WRITE_ONLY', () => {
			const image = cl.createImage(context, cl.MEM_WRITE_ONLY, imageFormat, imageDesc);
			cl.releaseMemObject(image);
		});
		...
		it('creates an image - cl.MEM_ALLOC_HOST_PTR', () => {
			const image = cl.createImage(context, cl.MEM_ALLOC_HOST_PTR, imageFormat, imageDesc);
			cl.releaseMemObject(image);
		});
		...
		it('creates an image - cl.MEM_HOST_READ_ONLY', () => {
			const image = cl.createImage(context, cl.MEM_HOST_READ_ONLY, imageFormat, imageDesc);
			cl.releaseMemObject(image);
		});

		it('creates an image - cl.MEM_HOST_NO_ACCESS', () => {
			const image = cl.createImage(context, cl.MEM_HOST_NO_ACCESS, imageFormat, imageDesc);
			cl.releaseMemObject(image);
		});

		it('throws if context is invalid', () => {
			assert.throws(() =>
				cl.createImage({} as unknown as cl.TClContext, 0, imageFormat, imageDesc),
			);
		});
		...
		it('throws cl.INVALID_IMAGE_DESCRIPTOR if image_desc is not valid or null', () => {
			assert.throws(
				() => cl.createImage(context, 0, imageFormat, -1 as unknown as cl.TClImageDesc),
				new Error('Argument 3 must be of type `Object`'),
			);
		});
		...
		it('throws cl.INVALID_IMAGE_FORMAT_DESCRIPTOR if image_format is invalid', () => {
			assert.throws(
				() => cl.createImage(context, 0, -1 as unknown as cl.TClImageFormat, imageDesc),
	...
	describe('#retainMemObject', () => {
		...
		it('throws if mem object is invalid', () => {
			assert.throws(() => cl.retainMemObject({} as unknown as cl.TClMem));
		});
	});
	...
	describe('#releaseMemObject', () => {
		...
		it('throws if mem object is invalid', () => {
			assert.throws(() => cl.releaseMemObject({} as unknown as cl.TClMem));
		});
	});
});
